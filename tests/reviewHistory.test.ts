...
const xml = `<SchoolUpload xmlns="http://ontario.ca"><Metadata/><School><SchoolNumber>123</SchoolNumber><Name>Example</Name><Students><Student><Name><First>Ada</First><Last>Example</Last></Name><Guardian><Name/></Guardian><Guardian><Name><First>Ann</First></Name><Relationship>MOTHER</Relationship></Guardian><Address><City>Guelph</City><Province>ON</Province></Address></Student></Students></School></SchoolUpload>`;
...
describe("review action history", () => {
  it("groups each apply operation and restores XML, findings and audit together", () => {
    ...
    const applied = apply(source, [change(source, "City", "Guelph", "Fergus"), change(source, "Province", "ON", "QC")]);
  ...
  it("undoes repeated changes in reverse order and retains undone entries after a new action", () => {
    const source = initial();
    const first = apply(source, [change(source, "City", "Guelph", "Fergus")], "Automatic fixes");
    const second = apply(first, [change(first, "City", "Fergus", "Elora")], "Cleaning mappings");
    ...
    const third = apply(undone, [change(undone, "City", "Fergus", "Toronto")]);
  ...
  it("replays separate batches when a removal changes Guardian positions", () => {
    ...
    const later = apply(edited, [change(edited, "City", "Guelph", "Fergus")]);
  ...
  it("revalidates under the current rules without undoing ruleset selection", () => {
    const source = initial();
    const applied = apply(source, [change(source, "City", "Guelph", "Fergus")]);
  ...
  it("rejects undo while corrections are pending", () => {
    const source = initial();
    const applied = apply(source, [change(source, "City", "Guelph", "Fergus")]);
    expect(() => undoLastReviewAction({ ...applied, fixes: [...applied.fixes, change(applied, "City", "Fergus", "Elora")] })).toThrow("Finish applying");
  });
});
