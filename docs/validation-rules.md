...
already identifies the same route. The repair clears the source street field
and writes the normalized value, such as `RR 1`, to `RuralRoute`. A conflicting
existing `RuralRoute` remains a manual review.
...
|---|---|---|
| `RURAL_ROUTE_IN_STREET_FIELD` | Yes when unambiguous and non-conflicting | Move to `RuralRoute` and normalize to `RR n` |
