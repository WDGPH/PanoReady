...
export const severityHelp: Record<ValidationSeverity, string> = {
  ...
  info: "Does not block submission. A safe cosmetic fix is available.",
};
...
export function SeverityLabel({ severity, label }: { severity: ValidationSeverity; label: string }) {
  return (
    <span
      tabIndex={0}
      title={severityHelp[severity]}
      aria-label={`${label}: ${severityHelp[severity]}`}
...
export function SeverityBadge({ severity }: { severity: ValidationSeverity }) {
  const config = {
    ...
    warning: { color: "var(--color-warning-text)", label: "Warning" },
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 500, color: config.color, letterSpacing: "0.02em", whiteSpace: "nowrap" as const }}>
      ...
      <SeverityLabel severity={severity} label={config.label} />
    </span>
  );
}
