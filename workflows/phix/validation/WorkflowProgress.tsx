const stages = ["Issues", "Auto Fix", "Manual Fix", "Download"];
  ...
  const overview = <ol className="workflow-progress-list">
    {stages.map((label, index) => {
      ...
      return <li key={label} aria-current={number === stage ? "step" : undefined}>
        {canNavigate(number) ? <button type="button" className="workflow-step-link" aria-label={`Return to ${label}`} onClick={() => onNavigate(number)} title={`Return to ${label}`}>{content}</button> : content}
      </li>;
    })}
  </ol>;
  return <nav className="workflow-progress" aria-label="PHIX fix progress">
