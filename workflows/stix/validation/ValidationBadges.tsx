...
export function SeverityBadge({ severity }: { severity: ValidationSeverity }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "var(--severity-font, var(--font-mono))", fontSize: 11, fontWeight: 500, color: config.color, letterSpacing: "var(--severity-tracking, 0.02em)", whiteSpace: "nowrap" as const }}>
...
export function GateBadge({ gate }: { gate: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, border: `1px solid ${color}`, borderRadius: 3, padding: "4px 12px", fontFamily: "var(--severity-font, var(--font-mono))", fontSize: 12, fontWeight: 500, letterSpacing: "0.03em", color }}>
      {icon}{gate}
    </span>
  );
}
